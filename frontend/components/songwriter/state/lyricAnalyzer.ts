import type { LyricLine, SyllableStress, RhymeScheme, SectionLyrics, SectionType, EmotionalTone, NarrativePerspective, VerbTense } from '../types/song'
...
export function analyzeSectionLyrics(
  ...
  const lines = linesText.map((text, index) =>
    parseLyricLineWithAnalysis(text, index, linesText)
  )
...
function detectNarrativePerspective(lines: ReadonlyArray<string>): NarrativePerspective {
  ...
  const firstPersonCount = firstPersonWords.reduce((count, word) =>
    count + (text.match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0), 0
  )

  const secondPersonCount = secondPersonWords.reduce((count, word) =>
    count + (text.match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0), 0
  )
...
function detectVerbTense(lines: ReadonlyArray<string>): VerbTense {
  ...
  const pastCount = pastIndicators.reduce((count, word) =>
    count + (text.match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0), 0
  )

  const presentCount = presentIndicators.reduce((count, word) =>
    count + (text.match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0), 0
  )

  const futureCount = futureIndicators.reduce((count, word) =>
    count + (text.match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0), 0
  )
...
function detectEmotionalTone(lines: ReadonlyArray<string>): EmotionalTone {
  ...
  for (const [tone, keywords] of Object.entries(emotionalKeywords)) {
    const score = keywords.reduce((count, word) =>
